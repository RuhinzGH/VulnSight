...
function NavigationWrapper({ currentTime, currentDate }) {
  ...
  return <Navigation currentTime={currentTime} currentDate={currentDate} navigate={navigate} />;
}
...
function App() {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setCurrentDate(`${String(now.getMonth() + 1).padStart(2, "0")}/${String(now.getDate()).padStart(2, "0")}/${now.getFullYear()}`);
    };
    tick();
            ...
            <div className="min-h-screen">
              ...
              <div className="container mx-auto py-8 px-4 max-w-md">
                <div className="tab-container relative flex mb-6">
                  <div className="tab-slider absolute bottom-0 h-0.5 bg-white transition-all duration-300 ease-in-out" style={sliderStyle} />
                  <button ref={scanRef} className={`tab-button font-bold ${activeTab === "scan" ? "active" : ""}`} onClick={() => setActiveTab("scan")}>
                    Scan
                  </button>
                  <button ref={resultsRef} className={`tab-button font-bold ${activeTab === "results" ? "active" : ""}`} onClick={() => setActiveTab("results")}>
                    Results
                  </button>
                </div>
                  ...
                  <>
                    {scanResults.vulnerabilities?.length > 0 ? (
                      <>
                        ...
                        <VulnerabilityResults results={scanResults.vulnerabilities} onViewDetails={setShowVulnerabilityDetails} />
                          ...
                          <div className="ai-summary-card mt-4">
                            ...
                            <ReactMarkdown>
                              {typeof scanResults.llm_summary === "string"
                                ? scanResults.llm_summary
                                : scanResults.llm_summary.explanation || JSON.stringify(scanResults.llm_summary, null, 2)}
                            </ReactMarkdown>
                          </div>
                        )}
                      </>
