...
async def scan_url(payload: ScanRequest, request: Request):
    ...
    async def explain_vulnerabilities_text(raw_results: dict) -> str:
        ...
        if not GROQ_API_KEY:
            print("⚠️ GROQ_API_KEY not configured; skipping LLM call.")
            return "LLM not configured."

        try:
            tools_list = normalize_results(raw_results)
            prompt = (
                "You are a cybersecurity expert. For each of the following findings, provide a short (1-3 sentence) "
                "plain-language summary of the risk and a concise remediation step. Keep it structured and readable.\n\n"
            )
